...
const NodeInfo = () => {
  const { isDialogOpen, closeDialog, nodeInfo, updateNodeInfo, saveEditedNode } = useNodeFlowContext();
        ...
        >
          ...
          <section className="flex flex-col gap-4">
            <fieldset className="flex flex-col gap-2 text-[12px] text-brand-text-dark">
              ...
              <DefaultSelectBox list={SELECT_BITCOIN_CORE_BINARY_VERSIONS} value={nodeInfo?.data?.version} placeholder="Select a bitcoin core version" updateSelection={(data) => updateNodeInfo("version", data.data)}/>
              {/* <p>{`Or choose the "start from scratch"`}</p> */}
            </fieldset>
            <fieldset className="flex flex-col gap-2 text-[12px] text-brand-text-dark">
              <label>Latency</label>
              <DefaultSelectBox list={SELECT_NODE_LATENCY} value={nodeInfo?.data?.latency} placeholder="Select node latency" updateSelection={(data) => updateNodeInfo("latency", data.data)}/>
            </fieldset>
            <fieldset className="flex flex-col gap-2 text-[12px] text-brand-text-dark">
            ...
            <fieldset className="flex flex-col gap-2 text-[12px] text-brand-text-dark">
              <label>RAM</label>
              <DefaultSelectBox list={SELECT_RAM} value={nodeInfo?.data?.ram?.toString()} placeholder="Select RAM size" updateSelection={(data) => updateNodeInfo("ram", data.data)}/>
            </fieldset>
            <fieldset className="flex flex-col gap-2 text-[12px] text-brand-text-dark">
              <label>CPU</label>
              <DefaultSelectBox list={SELECT_CPU_CORES} value={nodeInfo?.data.cpu?.toString()} placeholder="Select CPU core" updateSelection={(data) => updateNodeInfo("cpu", data.data)}/>
            </fieldset>
            <fieldset className="flex flex-col gap-2">
              <label className="text-xs">Node Name</label>
              <input id="network_name" value={nodeInfo?.data?.label} onChange={(e) => updateNodeInfo("label", e.target.value)} placeholder="My Network" className="h-[45px] px-[15px] w-[280px] border-b-[1px] border-brand-gray-medium bg-brand-gray-dark placeholder:text-sm placeholder:text-brand-text-dark" />
            </fieldset>
          </section>
