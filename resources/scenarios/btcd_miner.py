...
class BtcdMiner(Commander):
    ...
    def _log_status(self, round_num: int, status: dict):
        ...
        self.log.info("├────────────────┬──────────┬─────────┬─────────────┤")
        self.log.info("│  Node          │  Height  │  Peers  │  Mempool    │")
        self.log.info("├────────────────┼──────────┼─────────┼─────────────┤")
        for name, data in status.items():
            if "error" in data:
                self.log.info(f"| {name:<14} | ERROR    |         | {data['error'][:11]:<11} |")
            else:
                self.log.info(
                    f"| {name:<14} | {data['height']:>8} | {data['peers']:>7} | {data['mempool']:>11} |"
                )
        self.log.info("└────────────────┴──────────┴─────────┴─────────────┘")
