...
class SignetMinerScenario(Commander):
    ...
    def run_test(self):
        args = self.options
        args.bcli = lambda method, *args, **kwargs: self.nodes[self.options.tank].__getattr__(
            method
        )(*args, **kwargs)
...
def do_createpsbt(block, signme, spendme):
    psbt = PSBT()
    psbt.g = PSBTMap(
        {PSBT_GLOBAL_UNSIGNED_TX: signme.serialize(), PSBT_SIGNET_BLOCK: block.serialize()}
    )
    psbt.i = [
        PSBTMap(
            {
                PSBT_IN_NON_WITNESS_UTXO: spendme.serialize(),
...
def finish_block(block, signet_solution, grind_cmd):
    block.vtx[0].vout[-1].scriptPubKey += CScriptOp.encode_op_pushdata(
    ...
    if grind_cmd is None:
        ...
        newheadhex = subprocess.run(
            cmd, stdout=subprocess.PIPE, input=b"", check=True
        ).stdout.strip()
...
def get_reward_address(args, height):
    ...
    remove = [k for k in args.derived_addresses if k + 20 <= height]
    for k in remove:
...
def target_to_nbits(target):
    ...
    return int(f"{sz:02x}{fix:06x}", 16)
...
def do_generate(args):
    ...
    if args.min_nbits:
        if args.nbits is not None:
            ...
            return 1
        args.nbits = "1e0377ae"
        logging.info(f"Using nbits={args.nbits}")

    if args.set_block_time is None and (args.nbits is None or len(args.nbits) != 8):
    ...
    while max_blocks is None or mined_blocks < max_blocks:
        ...
        if lastheader is None:
            ...
            next_delta = next_block_delta(
                int(bestheader["bits"], 16),
                bestheader["hash"],
                ultimate_target,
                args.poisson,
                args.max_interval,
            ...
            logging.info(
                "Received new block at height %d; next in %s (%s)",
                bestheader["height"],
                seconds_to_hms(next_delta),
                ("mine" if next_is_mine else "backup"),
            )
            lastheader = bestheader["hash"]
        ...
        if now < action_time:
            ...
            logging.debug(
                f"Sleeping for {seconds_to_hms(sleep_for)}, next block due in {seconds_to_hms(mine_time - now)} ({minestr})"
            )
        ...
        if tmpl["previousblockhash"] != bci["bestblockhash"]:
            logging.warning(
                "GBT based off unexpected block (%s not %s), retrying",
        ...
        if tmpl["mintime"] > mine_time:
            ...
            if mine_time > now:
                logging.error(
                    "GBT mintime is in the future: %d is %d seconds later than %d",
                    mine_time,
                    (mine_time - now),
                    now,
                )
        ...
        # mine block
        logging.debug(
            "Mining block delta=%s start=%s mine=%s",
            seconds_to_hms(mine_time - bestheader["time"]),
            mine_time,
            is_mine,
        )
        mined_blocks += 1
        ...
        if not psbt_signed.get("complete", False):
            logging.debug(f"Generated PSBT: {psbt}")
        ...
        next_delta = next_block_delta(
            block.nBits, block.hash, ultimate_target, args.poisson, args.max_interval
        )
        ...
        logging.info(
            "Mined %s at height %d; next in %s (%s)",
            bstr,
            tmpl["height"],
            seconds_to_hms(next_delta),
            ("mine" if next_is_mine else "backup"),
        ...
        if r != "":
            logging.warning(
                "submitblock returned %s for height %d hash %s", r, tmpl["height"], block.hash
            )
...
def do_calibrate(args):
    ...
    for i in range(TRIALS):
        ...
        subprocess.run(cmd, stdout=subprocess.PIPE, input=b"", check=True).stdout.strip()
...
def get_args(parser):
    ...
    generate.add_argument(
        "--max-blocks", default=None, type=int, help="Max blocks to mine (default=1)"
    )
    generate.add_argument(
        "--set-block-time", default=None, type=int, help="Set block time (unix timestamp)"
    )
    generate.add_argument(
        "--nbits", default=None, type=str, help="Target nBits (specify difficulty)"
    )
    generate.add_argument(
        "--min-nbits", action="store_true", help="Target minimum nBits (use min difficulty)"
    )
    ...
    generate.add_argument(
        ...
        default=None,
        type=str,
        help="Specify which set of blocks to mine (eg: 1-40/100 for the first 40%%, 2/3 for the second 3rd)",
    )
    generate.add_argument(
        "--backup-delay",
        default=300,
        type=int,
        help="Seconds to delay before mining blocks reserved for other miners (default=300)",
    )
    generate.add_argument(
        "--standby-delay",
        default=0,
        type=int,
        help="Seconds to delay before mining blocks (default=0)",
    )
    generate.add_argument(
        "--max-interval", default=1800, type=int, help="Maximum interblock interval (seconds)"
    )
    ...
    for sp in [genpsbt, generate]:
        sp.add_argument(
            "--address", default=None, type=str, help="Address for block reward payment"
        )
        sp.add_argument(
            "--descriptor", default=None, type=str, help="Descriptor for block reward payment"
        )
    ...
    for sp in [solvepsbt, generate, calibrate]:
        sp.add_argument(
            ...
            default=None,
            type=str,
            required=(sp == calibrate),
            help="Command to grind a block header for proof-of-work",
        )

    args = parser.parse_args(sys.argv[1:])

    args.bcli = lambda *a, input=b"", **kwargs: bitcoin_cli(
        args.cli.split(" "), list(a), input=input, **kwargs
    )
