...
class ReplacementCyclingTest(WarnetTestFramework):
    ...
    def test_replacement_cycling(self):
        ...
        ab_funding_tx = generate_funding_chan(
            wallet, coin_1, alice_seckey.get_pubkey(), bob_seckey.get_pubkey()
        )
        ...
        bob_parent_txid = bob.sendrawtransaction(
            hexstring=bob_parent_tx.serialize().hex(), maxfeerate=0
        )
        bob_child_txid = bob.sendrawtransaction(
            hexstring=bob_child_tx.serialize().hex(), maxfeerate=0
        )
        ...
        alice_timeout_txid = alice.sendrawtransaction(
            hexstring=alice_timeout_tx.serialize().hex(), maxfeerate=0
        )
        self.log.info(
        ...
        bob_preimage_txid = bob.sendrawtransaction(
            hexstring=bob_preimage_tx.serialize().hex(), maxfeerate=0
        )
        self.log.info(
        ...
        self.log.info(
            f"@{last_blockheight} {bob_child_txid[0:7]} Child Txn "
        ...
        self.log.info(
            f"@{last_blockheight} {bob_preimage_txid[0:7]} Preimage Txn "
        ...
        self.log.info(
            f"@{last_blockheight} {bob_parent_txid_2[0:7]} Parent Txn 2 " f"- Broadcasted by: Bob"
        )
        ...
        self.log.info(
            f"@{last_blockheight} {bob_child_txid_2[0:7]} Child Txn 2 " f"- Broadcasted by: Bob"
        )
