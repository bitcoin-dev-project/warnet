...
def version_cmp_ge(version_str, target_str):
    ...
    # handle custom versions
        ...
        logger.info(ValueError(f"Unknown version string: {version_str} or {target_str} could not be compared"))
        logger.info("Assuming custom version can handle `addpeeraddress`")
        logger.debug(e)
