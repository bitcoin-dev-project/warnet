...
from .network import NETWORK_FILE, DEFAULTS_FILE as NETWORK_DEFAULTS_FILE, BITCOIN_CHART_LOCATION as NETWORK_CHART_LOCATION
from .namespaces import NAMESPACES_FILE, DEFAULTS_FILE as NAMESPACES_DEFAULTS_FILE, BITCOIN_CHART_LOCATION as NAMESPACES_CHART_LOCATION
...
def validate_directory(ctx, param, value):
    ...
    if not (directory / NETWORK_FILE).exists() and not (directory / NAMESPACES_FILE).exists():
        raise click.BadParameter(f"'{value}' does not contain a valid network.yaml or namespaces.yaml file.")
    return directory
...
def deploy(directory):
    ...
    if (directory / NETWORK_FILE).exists():
        ...
        click.echo("Error: Neither network.yaml nor namespaces.yaml found in the specified directory.")
...
def deploy_namespaces(directory: Path):
    ...
    for n in names:
        if not n.startswith("warnet-"):
            click.echo(f"Failed to create namespace: {n}. Namespaces must start with a 'warnet-' prefix.")
            return
